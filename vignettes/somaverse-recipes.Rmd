...
new <- log10(sim_test_data) |>
  purrr::modify_at(
    SomaDataIO::getAnalytes(sim_test_data), # just features
    cs)                                     # center/scale

# check means of feature data
apply(stripMeta(new), 2, mean) |> sum()  # mean = 0

# check sd of feature data
apply(stripMeta(new), 2, sd)
```
...
SomaDataIO::diffAdats(new, rcp_data)
```
...
identical(unname(stripMeta(new)), unname(stripMeta(rcp_data)))
```
...
`centerScaleData()`, which was previously in common use throughout the
`somaverse`.

## `centerScaleData()`

```{r center-scale-data}
cs_data <- log10(train) |> centerScaleData()

# check new class
is.centerScaled(cs_data)

# get the parameter table
pars <- attr(cs_data, "par_tbl")

# center/scale test based on train parameters
processed <- log10(test) |> centerScaleData(pars)
...
SomaDataIO::diffAdats(processed, soma_data)
```
...
SomaDataIO::diffAdats(old, log10(test))
