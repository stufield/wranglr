...
```{r imputing}
# Identify outliers (`globalr::get_outliers()`)
...
```{r seqLookup}
seqs <- withr::with_seed(101, sample(names(sample.adat), 10))
seqs
...
seqLookup(seqs, splyr:::apt_data)

# OR: on the fly
seqLookup(seqs, attr(sample.adat, "Col.Meta"))
```
