...
# Testing --------
test_that("`matchSamples()` has expected output", {
  out_og <- matchSamples(train, new, idcol = "SampleId")
  ...
  out_df <- matchSamples(as.data.frame(train), as.data.frame(new))
  expect_equal(out_df, expected)

  # check mixture of data.frame and soma_adat
  expect_equal(matchSamples(as.data.frame(train), new),
               list(x = expected$x, y = out_og$y))
  expect_equal(matchSamples(train, as.data.frame(new)),
               list(x = out_og$x, y = expected$y))
})

test_that("`matchSamples()` works with different sample ID inputs", {
  # use a different sample ID column
  train$id       <- train$SampleId
  new$id         <- new$SampleId
  train$SampleId <- new$SampleId <- NULL

  out_diffid <- matchSamples(train, new, idcol = "id")
...
test_that("`matchSamples()` throws errors/warnings as appropriate", {
  ...
  expect_error(matchSamples(bind_rows(train, train), new),
